...
class Spot{
public:
  Piece * P;       //Object Sclicing Purpose
        ...
        if (i%2==0) {
            ...
            for (int j=0; j<8; j++) {
                if (j%2==0) {
                    ...
                    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), blackBaground(chessBoard[i][j].getPiece()->team));
                    cout<<sell<<symbol[chessBoard[i][j].getPiece()->pieceatxy]<<sell;
                }
            }
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15);
    ...
    Game obj;
     ...
     int a,b,c,d;
         ...
         if(obj.check(a,b,c,d,ch))
         {
             ...
             if(chessBoard[a][b].getPiece()->isMoveValid(a,b,c,d))
             {
         ...
         for(int i=0;i<8;i++) for(int j=0;j<8;j++) if(chessBoard[i][j].getPiece()->pieceatxy==4) flag++;

         if(flag==2) goto label1;
         else
         {
             if(ch=='b') cout<<"\n BLACK WINS \n";
             else cout<<"\nWHITE WINS\n";
